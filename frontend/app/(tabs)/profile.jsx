...
const profile = () => {
  ...
  const [uploadStatusImage, setUploadStatusImage] = useState({
    loading: false,
    error: null,
    success: false,
  }); // State to track upload status
  const [uploadStatusLicense, setUploadStatusLicense] = useState({
    loading: false,
    error: null,
    success: false,
  }); // State to track upload status
  ...
  const {
    ...
    `http://${process.env.EXPO_PUBLIC_IP_ADDRESS}:3000/api/v1/user/userProfile`,
  );
  ...
  useEffect(() => {
    const handleUploadProfileImg = async () => {
      ...
      const fileType = uploadImageUri.substring(
        uploadImageUri.lastIndexOf(".") + 1,
      );
      ...
      try {
        const response = await axios.post(
          `http://${process.env.EXPO_PUBLIC_IP_ADDRESS}:3000/api/v1/user/uploadProfileImg`,
          formData,
          {
            headers: {
  ...
  const handleSaveChanges = async () => {
    ...
    const frontFileType = licenseFrontImg.substring(
      licenseFrontImg.lastIndexOf(".") + 1,
    );
    const backFileType = licenseBackImg.substring(
      licenseBackImg.lastIndexOf(".") + 1,
    );
    ...
    try {
      ...
      const response = await axios.post(
        `http://${process.env.EXPO_PUBLIC_IP_ADDRESS}:3000/api/v1/user/uploadDrivingLicense`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Failed to refresh the data.");
    }
    setRefreshing(false);
  };
        ...
        <View className="mt-6 space-y-2 px-5 mb-10">
          <View className="  items-start  mb-6 ">
            ...
            <View className=" w-full flex-col mt-5 justify-center items-center">
              <View className="rounded-full  border-black">
                <TouchableOpacity
              ...
              <Text className="font-semibold text-xl text-[#0D0D0D] mt-4">
                {name}{" "}
              </Text>
            </View>
          </View>
          ...
          <View className="mt-5 flex-col gap-y-4">
            ...
            <View className="flex-col gap-y-2">
              <Text className="font-normal text-xs text-[#858585]">
                Phone Number
              </Text>
              <View className="flex-row px-4 py-3 space-x-2 justify-start items-center rounded-md bg-[#F9F9F9]">
                ...
                <View className="gap-x-1 flex-row items-center rounded">
                  <Text className="text-[10px] text-primary font-normal">
                    verified
                  </Text>
                  <Image
                    source={icons.verify}
            ...
            <View className="flex-col gap-y-2">
              ...
              <View className="flex-row justify-between items-center px-4 py-3  rounded-md bg-[#F9F9F9]  ">
                ...
                <TouchableOpacity className="bg-[#FFF0B5] p-1  rounded">
                  <Text className="text-[10px] text-[#4D3D02] font-normal">
                    Verify your mail
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            <View className="flex-col gap-y-2.5">
                ...
                <ImageBackground
                  source={
                    licenseFrontImg
                      ? { uri: licenseFrontImg }
                      : icons.docsUpload
                  } // Dynamically loaded background image URL
                  resizeMode="cover"
                  className="flex-1" // Full-screen cover
                >
                  <View className=" flex-col items-center justify-between bg-[#F9F9F9]/70 px-3 py-4 rounded-md">
                    <View className=" flex-col items-center justify-center mb-3 ">
                      <Image source={icons.docsUpload} className="w-7 h-7 " />
                      <Text className="font-normal text-sm text-[#1A1401]">
                        Upload front side{" "}
                      </Text>
                      <Text className="font-light text-[10px] text-[#1A1401]">
                        Only JPG, PNG with max sie 2MB
                      </Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => {
                        handleLicenseImageSelection("front");
                      }}
                    >
                      <View className="bg-black/50 justify-center items-center p-1 rounded-xl">
              ...
              >
                <ImageBackground
                  source={
                    licenseFrontImg ? { uri: licenseBackImg } : icons.docsUpload
                  } // Dynamically loaded background image URL
                  resizeMode="cover"
                  className="flex-1" // Full-screen cover
                >
                  <View className=" flex-col items-center justify-between bg-[#F9F9F9]/70 px-3 py-4 rounded-md">
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        handleLicenseImageSelection("back");
                      }}
                    >
                      <View className="bg-black/50 justify-center items-center p-1 rounded-xl">
                        <Text className="font-medium text-sm text-primary">
